...
export abstract class PlanEstudioBaseComponent {
  ...
  loadPlanesEstudio(queryComplement?: string): Promise<PlanEstudio[]> {
    return new Promise<any>((resolve, reject) => {
      let endpoint: string;
      if (queryComplement) {
  ...
  async organizarEspaciosSemestreActual(): Promise<any> {
    ...
    await this.dataSemestre[this.punteroSemestrePlan].getAll().then((espacios: any) => {
  ...
  async removeFromSemester(event: any) {
  ...
  //#region
  runValidations2SpacesAdding(event: any): Promise<object> {
    let result = {
      valid: true,
      error: null,
      showPopUp: false,
  ...
  validarPrerrequisitoSemestreActual(prerrequisito: any): Promise<any> {
    return new Promise((resolve) => {
  ...
  async validarPrerequisitosAgregar(event: any): Promise<boolean> {
