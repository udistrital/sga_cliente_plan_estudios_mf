...
import { EspacioSummary, PlanEstudioSummary } from 'src/app/models/plan_estudio_summary';
...
export class SummaryPlanesEstudioComponent implements OnInit {
  ...
  @Input('dataPlanes') dataPlanes!: PlanEstudioSummary | undefined;
  ...
  mouseHoverIn(event: any, req: string[]) {
    ...
    req.forEach(r => {
      let e: HTMLElement | null = document.getElementById(r);
  ...
  mouseHoverOut(event: any, req: string[]) {
    let ep: HTMLElement = event.target;
